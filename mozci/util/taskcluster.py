...
def get_artifact(task_id, path, old_deployment=False):
    """
    ...
    """
    response = _do_request(get_artifact_url(task_id, path, old_deployment))
    return _handle_artifact(path, response)


def list_artifacts(task_id, old_deployment=False):
    response = _do_request(get_artifact_url(task_id, "", old_deployment).rstrip("/"))
