...
def test_push_tasks_with_cached_uncompleted_tasks(responses, cache):
    rev = "abcdef"
    branch = "autoland"

    TASKS_KEY = "{}/{}/tasks".format(branch, rev)

    cached_tasks = [task.Task.create(id=1, label="test-task", state="running")]
    cache.put(
        TASKS_KEY,
        cached_tasks,
...
def test_push_tasks_with_cached_completed_tasks(responses, cache):
    rev = "abcdef"
    branch = "autoland"
    ...
    cached_tasks = [
        task.Task.create(id=1, label="test-task", result="passed", state="completed")
    ]
    cache.put(
        TASKS_KEY,
        cached_tasks,
...
def test_finalized_push_tasks_with_cache(monkeypatch, responses, cache):
    cache = config.cache
    rev = "abcdef"
    branch = "autoland"
    ...
    cached_tasks = [task.Task.create(id=1, label="test-task", result="passed")]
    cache.put(
        TASKS_KEY,
        cached_tasks,
        config["cache"]["retention"],
